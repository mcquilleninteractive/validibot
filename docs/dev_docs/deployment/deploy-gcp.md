...
Copy the complete app-specific callback from ChatGPT's app-management page;
`callback_id` is generated by ChatGPT and must not be invented. Leave the
setting empty until that URL is available: the migration job will skip only
the ChatGPT client and continue normally. A non-empty legacy or malformed URL
causes `ensure_oidc_clients` to stop with a configuration error.
