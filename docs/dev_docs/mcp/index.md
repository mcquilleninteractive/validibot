...
- Desktop Codex and Claude use constrained Dynamic Client Registration (DCR)
  for public clients. The optional hosted ChatGPT plugin can use a predefined
  public client. Neither form has a client secret; both use authorization code,
  PKCE S256, exact callbacks, and explicit consent.
- django-allauth owns login, consent,
  authorization/registration/token/revocation endpoints, PKCE, refresh
  rotation, token persistence, and both discovery documents. The
  RFC 8414 path is an alias of allauth's discovery view; Validibot only uses
  allauth's supported adapter hook for the MCP scope, exact resource and DCR
  policy, canonical public origin, and RFC 8707 refresh inheritance. The last hook
...
- Access tokens expire after 15 minutes by default. Rotating refresh tokens
  ...
  registration, token, and revocation endpoints have independent shared-cache
  IP/global ceilings before expensive OAuth work. Inactive DCR clients with no
  live token are eligible for conservative retention-based cleanup.
...
| `IDP_OIDC_DCR_ENABLED` | `True` | Advertise constrained public-client registration for desktop MCP clients. |
| `IDP_OIDC_DCR_HTTPS_REDIRECT_HOSTS` | `chatgpt.com,claude.ai` | Exact approved HTTPS callback hosts; native loopback HTTP is handled separately. |
| `IDP_OIDC_DCR_MAX_METADATA_BYTES` | `16384` | Maximum serialized registration metadata. |
| `IDP_OIDC_DCR_MAX_REDIRECT_URIS` | `8` | Maximum callbacks in one registration. |
| `IDP_OIDC_DCR_MAX_REDIRECT_URI_LENGTH` | `2048` | Maximum length of one callback URI. |
| `IDP_OIDC_DCR_INACTIVE_CLIENT_RETENTION_DAYS` | `30` | Retention for unused DCR clients with no live token. |
| `IDP_OIDC_CHATGPT_REDIRECT_URIS` | empty | Optional app-specific callback for the predefined hosted ChatGPT plugin client. It is not used by desktop Codex or Claude. |
...
| `IDP_OIDC_REGISTRATION_REQUESTS_PER_IP_PER_MINUTE` | `10` | OAuth client-registration budget per trusted client address. |
...
| `IDP_OIDC_ENDPOINT_GLOBAL_REQUESTS_PER_MINUTE` | `1000` | Combined deployment-wide registration/token/revocation ceiling. |
