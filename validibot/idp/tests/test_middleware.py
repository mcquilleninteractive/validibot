"""Tests for abuse controls on public OAuth client-lifecycle endpoints.

The MCP SDK transport limiter cannot see django-allauth's registration, token,
or revocation routes. These tests prove those CSRF-exempt endpoints have
independent, shared-cache IP and global ceilings before OAuth body processing.
"""
...
@pytest.mark.parametrize("endpoint_name", ["client_registration", "token", "revoke"])
def test_post_endpoints_enforce_per_ip_limits(settings, endpoint_name: str) -> None:
    ...
    settings.IDP_OIDC_REGISTRATION_REQUESTS_PER_IP_PER_MINUTE = 1
...
def test_global_limit_is_shared_across_oauth_endpoint_types(settings) -> None:
    """Rotating between registration and token routes must not reset the ceiling."""

    settings.IDP_OIDC_REGISTRATION_REQUESTS_PER_IP_PER_MINUTE = 100
    settings.IDP_OIDC_TOKEN_REQUESTS_PER_IP_PER_MINUTE = 100
    settings.IDP_OIDC_ENDPOINT_GLOBAL_REQUESTS_PER_MINUTE = 1
    request_factory = RequestFactory()
    middleware = OIDCEndpointAbuseProtectionMiddleware(
